...
axiom estimate_within_factor_2 (msg : ChatMessage) :
    (estimate_tokens_single msg).val ≤ 2 * (actual_tokens msg + MESSAGE_OVERHEAD.val)

/-- The total estimate over a list is within the sum of per-message bounds. -/
axiom estimate_total_bound (msgs : List ChatMessage) :
    (estimate_tokens msgs).val ≤
    2 * (msgs.map (fun m => actual_tokens m + MESSAGE_OVERHEAD.val)).foldl (· + ·) 0
