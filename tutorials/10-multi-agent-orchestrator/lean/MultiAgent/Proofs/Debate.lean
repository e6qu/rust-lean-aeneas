...
axiom debate_step_finished (state : DebateState) (agent_id : AgentId) (arg_id : U32)
...
axiom debate_step_decrements (state : DebateState) (agent_id : AgentId) (arg_id : U32)
    (state' : DebateState)
...
axiom debate_step_adds_argument (state : DebateState) (agent_id : AgentId) (arg_id : U32)
    (state' : DebateState)
    (h_pos : state.rounds_remaining.val > 0)
    (h_step : debate_step state agent_id arg_id = .ok state') :
    state'.arguments.length = state.arguments.length + 1
