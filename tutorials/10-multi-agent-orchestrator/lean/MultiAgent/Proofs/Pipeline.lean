...
axiom pipeline_head_types_match (s1 s2 : PipelineStage) (rest : List PipelineStage)
    (h : is_pipeline_valid_aux (s1 :: s2 :: rest) = true) :
    s1.output_type = s2.input_type

/-- If `is_pipeline_valid_aux (s1 :: s2 :: rest)` is true,
    then `is_pipeline_valid_aux (s2 :: rest)` is also true. -/
axiom pipeline_tail_valid (s1 s2 : PipelineStage) (rest : List PipelineStage)
    (h : is_pipeline_valid_aux (s1 :: s2 :: rest) = true) :
    is_pipeline_valid_aux (s2 :: rest) = true

/-- If `is_pipeline_valid pipeline = true`, then for all consecutive stages
    `(i, i+1)`, `stages[i].output_type = stages[i+1].input_type`. -/
axiom valid_pipeline_types_match (pipeline : Pipeline)
